...
command! Change call P4change()
command! CHange Change
command! Ch Change
command! CH Change
nnoremap \c :Change<CR>

function! ChangeDesc(changenum)
    tabnew
    execute "read !p4 describe -s " . a:changenum
endfunction
command! -nargs=1 Chd call ChangeDesc(<f-args>)
command! -nargs=1 CHd call ChangeDesc(<f-args>)
command! -nargs=1 CHD call ChangeDesc(<f-args>)
nnoremap \d :call ChangeDesc(expand("<cword>"))<CR>


function! Blame()
    let linenum = line(".")
    let filename=@%
    let cmd="read !p4blame " . filename
    tabnew
    execute cmd
    g/^$/d
    "cal cursor(linenum, 0)
    let ncmd = "normal! " . (linenum + 2) . "Gzz"
    execute ncmd
endfunction
